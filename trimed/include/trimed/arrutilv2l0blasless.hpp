...
inline void proxy_openblas_set_num_threads(int nthreads) { (void)nthreads; }

template <typename TInt, typename TFloat>
void set_rargmaxabs(TInt nrows, TInt ncols, const TFloat *const A,
                    TInt *argmaxabss) {
  TFloat absval;
  TFloat vmax;
...
template <typename TInt, typename TFloat>
void rank1rowupdate(TInt ncols, const TFloat *const row, TFloat scale,
                    TInt nrows, TFloat *const toupdate) {
...
template <typename TInt, typename TFloat>
inline void set_l22(const TInt &dimension, const TFloat *const a,
                    const TFloat *const b, const TFloat &a_l22,
                    const TFloat &b_l22, TFloat &l22) {
  l22 = 0;
...
template <typename TInt, typename TFloat>
void set_l22s(TInt nrows, TInt ncols, const TFloat *const A, TFloat *const l22s,
              bool byrow) {
  if (byrow == true) {
    for (TInt r = 0; r < nrows; ++r) {
...
template <typename TInt, typename TFloat>
void set_sums(TInt nrows, TInt ncols, const TFloat *const A, TFloat *const sums,
              bool byrow) {
  if (byrow == true) {
    for (TInt r = 0; r < nrows; ++r) {
...
* */
template <typename TInt, typename TFloat>
inline void set_rl22s(const TInt &ncols, const TFloat *const v,
                      const TInt &nrows, const TFloat *const B,
                      const TFloat &v_l22s, const TFloat *const B_l22s,
                      TFloat *const l22s) {
  for (TInt r = 0; r < nrows; ++r) {
    l22s[r] = 0;
    for (TInt c = 0; c < ncols; ++c) {
...
void set_rrl22ss(TInt nrowsA, TInt ncols, const TFloat *const A, TInt nrowsB,
  ...
  for (TInt r = 0; r < nrowsA; ++r) {
    set_rl22s(ncols, A + r * ncols, nrowsB, B, A_l22s[r], B_l22s,
              l22ss + r * nrowsB);
  }
}
