...
// TODO : think about when to return std::unique_ptr< []> and when to return
// std::vector

namespace arrutilv2 {

template <typename TInt, typename TLabel>
std::unique_ptr<TLabel[]> get_vhisto(TInt nrows, TInt ncols, TInt range,
                                     const TLabel *const labels) {
...
template <typename TInt, typename TLabel>
std::vector<TInt> get_where_above_threshold(TInt N, const TLabel *const X,
                                            TInt threshold) {
  std::vector<TInt> where;
  for (TInt i = 0; i < N; ++i) {
    if (X[i] > threshold) {
...
void set_maxinpartition(TInt npts, TInt npartitions, const TNumber *const vals,
                        TNumber *const maxinpartition) {
  TInt p_end = 0;
  TInt p_start;
...
/* same as inline void set_minexclusionnocheck in arrutilv2l1.hpp> */
template <typename TInt, typename TNumber>
void set_min_excluding(TInt nvals, const TNumber *const vals, TInt excl,
                       TNumber &toset) {
...
void make_balanced(TSize minclustersize, TSize ndata, TLabel *const L,
                   TSize nclusters, TSize *const groupsizes) {
  ...
  while (minc < minclustersize) {
    if (maxc <= minclustersize) {
      throw std::runtime_error(
          "In get contig by cluster 3, trying to balance clusters. minc < "
          "minclustersize, so balancing required. But maxc <= minclustersize, "
          "so balancing will cause another hole to appear, it will be "
          "impossible to fill all the holes there are simply not enough plugs");
    }
